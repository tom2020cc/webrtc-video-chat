...
(function(){
 ...
 function matchingVoices(){return (synth?.getVoices()||[]).filter(v=>v.lang.toLowerCase().replace('_','-').split('-')[0]===prefs.lang.split('-')[0]).filter(v=>['auto','all'].includes(prefs.gender)||(prefs.gender==='female'?female:male).test(v.name)).sort((a,b)=>Number(b.localService===(prefs.quality==='local'))-Number(a.localService===(prefs.quality==='local')));}
 function refreshVoices(){
  ...
  put('voiceReaderAvailability',choices.length?`找到 ${choices.length} 个声音 · ${languages[prefs.lang]}。自动匹配不保证男女声，具体音质以试听为准。`:'暂无匹配声音：点击「自动匹配」重新查找，或安装对应语言语音包。');
 ...
 function sync(){
  ...
  const quick=$('quickVoiceToggle');if(quick){quick.setAttribute('aria-pressed',String(enabled));quick.classList.toggle('on',enabled);quick.textContent=enabled?'🔊 朗读已开启':'🔈 开启译文朗读';}
 ...
 async function waitVoice(job){
  for(let n=0;n<10&&valid(job);n++){refreshVoices();const voice=selectedVoice();if(voice)return voice;if((synth?.getVoices()||[]).length)break;status('正在加载设备声音…');await new Promise(r=>setTimeout(r,400));}
  return null;
 }
 ...
 function setEnabled(value){cancel();enabled=Boolean(value&&synth);sync();if(enabled){refreshVoices();const voice=selectedVoice();if(voice){status('正在启用声音…');speak(samples[prefs.lang],voice).then(()=>drain());}else status('朗读已开启，但没有匹配声音；请点自动匹配');}else status('仅显示字幕，朗读已关闭');}
 ...
 function setPreference(key,value){cancel();prefs[key]=value;if(['lang','gender','quality'].includes(key))prefs.voice='';refreshVoices();save();status(enabled?'设置已更新，等待对方新字幕，或重播上一句':'设置已保存，点击开启朗读');sync();}
 ...
 function loadVoices(){refreshVoices();if(!(synth?.getVoices()||[]).length&&voicePolls++<8)setTimeout(loadVoices,500);}
 if(synth){synth.addEventListener('voiceschanged',refreshVoices);loadVoices();}else{status('此浏览器不支持朗读，请使用支持系统语音的浏览器');$('voiceReaderEnabled').disabled=true;$('voiceReaderPreview').disabled=true;}
